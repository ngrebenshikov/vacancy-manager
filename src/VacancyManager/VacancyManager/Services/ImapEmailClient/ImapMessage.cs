    ...
    internal class ImapMessage
    {
        ...
        public void SaveVMMailMessage()
        {
            ...
            if (ApplicantId != 0)
            {
                ...
                foreach (var consider in fromapp.Considerations)
                {
                    if (Subject.Contains(consider.Vacancy.Title))
                    { condsiderId = consider.ConsiderationID; }
                }

                int messageId = VMMailMessageManager.Create(From, To, Subject, Text, SendDate, DeliveryDate, 1, ApplicantId, condsiderId).Id;
